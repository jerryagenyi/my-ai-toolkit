@echo off
:: ============================================================
:: MODEL-SPECIFIC — Gemma4 12B Q4_K_M (terminal chat)
:: ============================================================
:: VRAM rule: terminal chat loads the model directly into VRAM.
:: It CANNOT run at the same time as the server (both need ~7.4 GB).
::
...
:RUN_CHAT
echo.
echo Loading Gemma4 12B Q4_K_M (~13 seconds)...
echo Type your message and press Enter. /bye or Ctrl+C to exit.
echo.
"%LLAMA_CLI%" --model "%MODEL_PATH%" -ngl %NGL% %EXTRA_ARGS% --ctx-size %CTX% -cnv -i
